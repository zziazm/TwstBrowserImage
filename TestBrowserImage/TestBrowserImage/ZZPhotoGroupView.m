...
- (instancetype)init {
    ...
    _progressLayer.bounds = CGRectMake(0, 0, 40, 40);//CGSizeMake(40, 40);
...
- (void)showHUD:(NSString *)msg {
    ...
    CGSize size = result;//[msg sizeForFont:font size:CGSizeMake(200, 200) mode:NSLineBreakByCharWrapping];
    UILabel *label = [UILabel new];
    CGFloat scale = [UIScreen mainScreen].scale;
    

    label.size = CGSizeMake(ceil(size.width * scale) / scale,
                            ceil(size.height * scale) / scale);//CGSizePixelCeil(size);
    label.font = font;
    label.text = msg;
...
//    }
    id imageItem = tile.imageView.image; //[tile.imageView.image imageDataRepresentation];
//    YYImageType type = YYImageDetectType((__bridge CFDataRef)(imageItem));
//    if (type != YYImageTypePNG &&
//        type != YYImageTypeJPEG &&
//        type != YYImageTypeGIF) {
